...
export class SolignitionClient {
   ...
   */
  async deposit(depositor: PublicKey, amount: BN) {
   ...
   */
  async withdraw(depositor: PublicKey, shares: BN) {
    const [depositorRecord] = this.getDepositorRecordPda(depositor);
    const [protocolConfig] = this.getProtocolConfigPda();
    const [vault] = this.getVaultPda();
    const [eventAuthority] = this.getEventAuthorityPda();
  ...
  async requestLoan(
    ...
    const [loan] = this.getLoanPda(config.loanCounter, borrower);
    const [vault] = this.getVaultPda();
    const [adminPda] = this.getAdminPda();
    const [eventAuthority] = this.getEventAuthorityPda();
   ...
   * Repay a loan
   */
  async repayLoan(borrower: PublicKey, loanId: BN) {
   ...
   */
  async setPaused(admin: PublicKey, isPaused: boolean) {
    const [protocolConfig] = this.getProtocolConfigPda();
    const [eventAuthority] = this.getEventAuthorityPda();
   ...
   */
  async claimAdmin(admin: PublicKey, treasury: PublicKey) {
    const [adminPda] = this.getAdminPda();
    const [protocolConfig] = this.getProtocolConfigPda();
    const [eventAuthority] = this.getEventAuthorityPda();
...
// Export helper function to confirm transaction
